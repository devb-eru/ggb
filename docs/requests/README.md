...
| [정본 런타임 오브젝트 대장](../game_object_catalog.md) | 독립 상태·저장 의미를 가진 62개 오브젝트 |
| [전체 제작 오브젝트 인벤토리](../game_object_inventory.md) | 정본 62개와 퍼즐·문서·UI·소품 제작 엔티티 |
...
| 배치 | 범위 | 현재 운영 |
| --- | --- | --- |
| `BATCH-VS01` | 20~30분 버티컬 슬라이스 | 상세 의뢰, 팀 수락 대기 |
...
| 범위 | 정본 | 의뢰 연결 |
| --- | ---: | --- |
| 아트 담당 에셋 | 228행 | VS 28 + 데모 잔여 115 + 본편 76 + 상점 9 = 228 |
| 사운드 담당 에셋 | 97행 | VS 19 + 데모 잔여 50 + 본편 28 = 97 |
| 기술 셰이더 | 4행 | `beru` 소유, ART/AUD 팀 의뢰 범위 밖 |
| 콘텐츠 원문 | EP00~EP09 | VS + 데모 4패키지 + 본편 6패키지 |
| 정본 오브젝트 | 62개 | 아트·CNT 전수 매핑 완료 |
| 비런타임 제작 엔티티 | 84묶음 | 전체 제작 오브젝트 인벤토리에서 부모 에셋 아래 추적 |

매니페스트 329행은 아트 228 + 사운드 97 + 기술 셰이더 4로 모두 소유자가 정해져 있다. 콘텐츠 원문은 매니페스트 에셋 행이 아니라 `content_item_id`와 승인 뒤 `line_id`로 관리한다.
...
- 의뢰 ID와 대장 중복.
- `READY_FOR_ACCEPTANCE` 상세 문서의 존재와 ID 일치.
