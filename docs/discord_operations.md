...
회의 안건은 GitHub Actions가 금요일 21:30 KST에 `GGB Production`을 읽어 다음 순서로 만든다. 같은 작업이 여러 조건에 걸리면 먼저 나오는 구역에 한 번만 표시한다.

1. 지금 결정하지 않으면 멈추는 `P0/P1`과 `BLOCKED`.
2. 다음 회의 전 마감되는 작업.
3. `REVIEW`에 들어간 뒤 24시간 넘게 검토 대기 중인 작업.
4. 일정 초과 작업.
5. 실제 담당자가 비어 있거나 `UNASSIGNED`인 활성 작업.

`REVIEW` 대기 시간은 Project 5분 감시 캐시에 보존한 상태 진입 시각으로 계산한다. 캐시가 처음 생기거나 기존 캐시에 진입 시각이 없으면 현재 시각부터 측정해 과거 시간을 추정하지 않는다. 자동 안건이 없어도 Project 링크가 포함된 준비 메시지를 게시한다.
...
- Discord Scheduled Event의 생성·수정·취소는 프로젝트 관리자 `beru`가 직접 수행한다. 자동화는 이벤트를 조작하지 않는다.
