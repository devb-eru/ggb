...
[제작 의뢰서](requests/README.md)는 만들기 전 범위·맥락·산출물·승인 기준을 합의한다. 본 문서는 제작 중간본이나 완성물을 받은 뒤 보관·검토·통합하는 절차다.

```text
REQ-* READY_FOR_ACCEPTANCE
→ 팀 수락 ACCEPTED
→ 제작
→ 본 문서의 INTAKE
→ REVIEW·APPROVED·INTEGRATED
```

납품은 원 요청의 `request_id`를 반드시 되돌려 준다. 긴급 수정처럼 선행 의뢰가 없었다면 `beru_pending`으로 접수할 수 있지만 `REVIEW` 전까지 요청 또는 결함 ID를 발급해 범위와 승인 조건을 복구한다.
...
| [`docs/requests`](requests/) | 제작 전 범위, 대상 ID, 팀 수락과 산출물 기준 | 의뢰 정본 |
...
- `request_id`: 선행 제작 의뢰 `REQ-ART-*`, `REQ-AUD-*`, `REQ-CNT-*`. 긴급 미발급은 `beru_pending`.
- `contributor`: 실제 제작자.
- `asset_type`: `art`, `audio`, `ui`, `content`, `source` 중 하나.
- `version`: `vNNN`.
...
1. 제작 팀장은 `request_id`의 수락 범위와 실제 납품 목록이 일치하는지, source가 다시 편집 가능한지, export가 요구 규격을 충족하는지 확인한다.
