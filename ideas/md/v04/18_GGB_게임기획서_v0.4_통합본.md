...
- 2026-07-24 정적 검사에서 H1, 코드 펜스, 상대 Markdown 링크와 엔딩 ID 참조가 통과했다. Mermaid는 텍스트 구조만 검사했으며 실제 렌더링은 후속 검증 대상이다.
...
모든 owner 색에는 문양·선 패턴·라벨·음향 자막이 병행된다. 핫스폿은 최소 44×44px이며 큰 타깃 56×56px을 지원한다. Godot에서는 `ObjectRegistry`, `ObjectReactionResolver`, `InteractionRouter`, `ObjectReactionMemory`, `HotspotAccessibilityAdapter`로 책임을 분리하고 저장 스키마 12를 사용한다.
