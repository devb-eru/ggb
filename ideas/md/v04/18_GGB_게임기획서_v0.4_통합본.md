...
매일 같은 고딕 저택에서 눈뜨는 주인공이 D5 이전 `NORMAL_RESET`을 이용해 세계의 물성을 거슬러 조사하고, 자신을 가둔 다섯 연구원 인격과 관계를 맺으며 황폐한 현실과 자각된 가상 낙원 사이에서 자기 선택권을 되찾는 게임.
...
- 2026-07-26 정적 검사에서 H1, 코드 펜스, 상대 Markdown 링크, Windows 플랫폼 표기, 색 계층과 접근성 프로필 참조가 통과했다. Mermaid는 텍스트 구조만 검사했으며 실제 렌더링은 후속 검증 대상이다.
