...
| 후반 경계 | FRACTURE_SLEEP, BROKEN_RESET(이벤트 ID), BROKEN_RESET_ONCE(단발 전환), POST_BROKEN_REST, FINAL_SLEEP_LOCK |
...
flowchart TB
    ...
    FS --> BR["BROKEN_RESET\n이벤트 ID"]
    BR --> BRO["BROKEN_RESET_ONCE\nS3 물리 월드 1회 생성"]
    BRO --> E1["E1 같은 침실의 다른 아침"]
```
...
```yaml
failure_record:
  failure_id: FAIL_B3_B_L0003_01
  source_event_id: B3_B
  status: active
  observed_result_id: B3_B_RELAY_AT_TWELFTH_BELL
  validated_steps:
...
| 상태 | 의미 |
| --- | --- |
| active | 다음 ROUTE와 숏컷이 읽음. source event마다 최대 하나 |
| resolved | 퍼즐 성공 뒤 기록 보관함으로 이동 |
| superseded | 더 높은 단계의 검증 정보가 대체 |

성공한 퍼즐의 과거 실패 기록은 삭제하지 않지만 현재 목표에는 표시하지 않는다.

- 같은 source event의 active 기록은 최대 하나다.
- 동일 정보로 다시 실패하면 `occurrence_count`와 `last_loop`만 갱신한다.
- 더 구체적인 검증 정보가 생기면 기존 active를 superseded로 바꾸고 새 기록을 active로 만든다.
- B3-B·C4·D1 성공 트랜잭션은 해당 source의 active 기록을 resolved로 바꾸고 연결된 `shortcut_resume`을 지운다.
- ROUTE는 `*_hard_failure_seen`이 아니라 active 기록을 읽으며, 스냅숏의 기록이 resolved·superseded면 안전 진입점으로 다시 판정한다.
...
prerequisites:
  all:
    ...
    - active_failure(source_event_id: B3_B)
outputs:
...
prerequisites:
  all:
    - journal_stage >= 2
    - c3_formula_verified
    - active_failure(source_event_id: C4)
outputs:
  create_loop_items:
    - CLEAN_CLOTH
    - WATER_MEASURE
...
prerequisites:
  all:
    ...
    - active_failure(source_event_id: D1)
outputs:
...
| 상태 | 구간 | `camouflage_filter_state` | 기능 |
| --- | --- | --- | --- |
| NORMAL_SLEEP | P~D4 | `ACTIVE` | 영구 저장 뒤 물리 초기화 |
| FRACTURE_SLEEP | D5 직후 | `DISABLED` | 정상 복구 시도 실패 |
| BROKEN_RESET | D6 | `DISABLED → BROKEN` | `BROKEN_RESET_ONCE`로 S3 다른 아침을 1회 생성 |
| POST_BROKEN_REST | E~F | `BROKEN` | 휴식·시간 전환, 물리 유지 |
| FINAL_SLEEP_LOCK | F3 진입 이후 | `BROKEN` | 선택 회피용 수면 비활성, 저장·조사·EDC 취소 허용 |

```yaml
sleep_router:
  if:
    - final_sleep_lock == true: FINAL_SLEEP_LOCK
    - camouflage_filter_state == BROKEN: POST_BROKEN_REST
    - camouflage_filter_state == DISABLED: FRACTURE_SLEEP
    - camouflage_filter_state == ACTIVE: NORMAL_SLEEP

broken_reset_transition:
...
post_broken_rest:
  ...
  preserves: [world_phase, object_states, servant_locations, E3_repair_states, relationship_state]
```

F3 경계:

```yaml
final_sleep_boundary:
  trigger: F3_ENTRY
  set: final_sleep_lock=true
  blocks: [NORMAL_SLEEP, FRACTURE_SLEEP, BROKEN_RESET, POST_BROKEN_REST]
  allows: [F3_INSPECTION, SAVE_LOAD, EDC_OPEN, EDC_CANCEL]
  release: none_in_current_run
```

F3 완료가 아니라 F3 진입이 수면 라우터를 잠근다. F3 완료는 장치 3종 조사와 EDC 활성만 담당한다.

D5 완료는 `ACTIVE → DISABLED`를 D5 완료 플래그와 함께 저장한다. `BROKEN_RESET_COMPLETION`은 `DISABLED → BROKEN`, `broken_reset_triggered=true`, `fracture_sleep_complete=true`, S3 생성을 전부 적용하거나 전부 롤백한다. 따라서 저장·로드 중단으로 `BROKEN`과 단발 리셋 완료 여부가 어긋나지 않는다.
...
- BROKEN_RESET의 `BROKEN_RESET_ONCE` 뒤 S3와 물리 색 채널이 유지된다.
- 이후 휴식은 `POST_BROKEN_REST`로만 처리하며 S3 템플릿을 다시 생성하지 않는다.
- 이후 NORMAL_RESET은 차단된다.
- E3 수리 뒤 POST_BROKEN_REST를 실행해도 현재 `object_states`, 사용인 위치, 수리 결과가 유지된다.
- F3_ENTRY에서 FINAL_SLEEP_LOCK이 활성화되고 EDC 취소 뒤에도 유지된다.
...
- F3 진입 뒤 세계 내 수면은 차단되지만 저장·F3 조사·EDC 취소는 유지된다.
...
## 36. 공통 오브젝트 반응의 루프 수명 주기

### 36.1 저장 계층

| 데이터 | NORMAL_RESET | BROKEN_RESET_ONCE | POST_BROKEN_REST |
| --- | --- | --- | --- |
| 물리 `object_state` | 아침 스냅샷으로 복원 | D5 결과를 적용한 S3 상태로 단발 전환 | 현재 상태 유지 |
| `loop_state.object_reaction_runtime.loop_counts` | 전부 초기화 | 초기화 후 S3 카운트 생성 | 유지 |
| `meta_progress.object_reaction_memory.persistent_seen` | 유지 | 유지 | 유지 |
| `meta_progress.object_reaction_memory.per_state_seen` | 같은 world phase면 유지 | S3용 새 첫 반응 허용 | 유지 |
| `knowledge_state`·수첩 | 유지 | 유지 | 유지 |
| 퍼즐 임시 입력 | 초기화 | D5 이전 임시 입력 폐기 | 현재 S3 사건 체크포인트 규칙 적용 |
| 관계·사용인 기억 | 유지 | 유지 | 유지 |

`per_state`는 “영구히 한 번”이 아니라 `(reaction_id, world_phase, object_state)` 조합의 첫 조사다. 따라서 같은 창문도 S1의 날씨 모순과 S3의 투영 반복을 각각 처음 발견할 수 있다.

### 36.2 NORMAL_RESET 트랜잭션

```text
NORMAL_SLEEP
→ SYS_COMMIT
  - persistent knowledge·reaction 이력 커밋
  - 현재 실패 정보 커밋
→ SYS_MEMORY
  - 사용인 residual memory 반영
→ NORMAL_RESET
  - 물리 오브젝트를 morning_snapshot으로 복원
  - object_reaction_runtime.loop_counts와 per-loop 표시 제거
→ MORNING
→ ROUTE
```

- 침대 `OBJ_BEDROOM_BED`는 위 체인을 요청하는 `SYSTEM_GATE`다.
- 오브젝트 반응 자체가 수면을 확정하지 않는다. 취침 확인에서 취소하면 어떤 reset 쓰기도 발생하지 않는다.
- `OBJ_EMERGENCY_REST_CAPSULE`은 D5 이후에만 FRACTURE_SLEEP을 요청하며 NORMAL_RESET으로 연결하지 않는다.

### 36.3 첫 조사·반복 조사

1. selector가 현재 `event_context`, `object_state`, `world_phase`, `knowledge_state`에 맞는 반응을 찾는다.
2. persistent·per-state 이력에 없으면 `first_text_id`를 사용한다.
3. 이미 확인했다면 `repeat_text_id`를 사용한다.
4. 반복 조사 횟수가 3회를 넘으면 문구를 더 짧게 줄이되 힌트 정보는 삭제하지 않는다.
5. 새로운 영구 정보가 생기면 같은 오브젝트라도 `knowledge_state`가 바뀐 새 반응을 허용한다.

### 36.4 숏컷과의 관계

- 숏컷 자격은 `object_reaction_memory.persistent_seen`이 아니라 `knowledge_id`, `failure_record`, `lock_state`, `journal_stage`로 판정한다.
- 숏컷 진입 시 이미 해결한 AMBIENT·INFO_RECORD 반응을 강제로 재생하지 않는다.
- 플레이어가 다시 조사하면 최신 지식에 맞는 축약 반응과 “왜 건너뛸 수 있는지”를 수첩에서 확인할 수 있다.
- BSHORT는 기록 시계 실패 정보, CSHORT는 코팅·혼합 실패 정보, DSHORT는 세 축의 잠금 깊이 정보를 읽는다. 오브젝트 클릭 횟수는 대체 조건이 아니다.
- 숏컷 뒤에도 퍼즐의 비가역 입력과 최종 확인은 생략하지 않는다.

### 36.5 D5 이후 휴식

```text
D5_complete
→ OBJ_BEDROOM_BED 또는 OBJ_EMERGENCY_REST_CAPSULE
→ FRACTURE_SLEEP
→ BROKEN_RESET_ONCE
→ E1

E1_complete 이후 같은 두 오브젝트
→ POST_BROKEN_REST
→ 현재 S3 공간·수리·사용인 위치 보존
```

첫 FRACTURE_SLEEP만 `BROKEN_RESET_ONCE`를 소비한다. 이후 휴식은 회복과 시간대 전환이며 세계를 아침 스냅샷으로 되돌리지 않는다.

### 36.6 루프 QA

| QA ID | 검증 |
| --- | --- |
| `QA-LOOP-OBJ-01` | NORMAL_RESET 뒤 물리 상태는 복원되고 수첩·사용인 기억은 유지 |
| `QA-LOOP-OBJ-02` | S1 첫 조사 완료 뒤 S3 첫 조사 문구가 정상 노출 |
| `QA-LOOP-OBJ-03` | 반복 조사만으로 어떤 숏컷도 해금되지 않음 |
| `QA-LOOP-OBJ-04` | D5 뒤 두 수면 오브젝트가 정확히 한 번만 BROKEN_RESET_ONCE 호출 |
| `QA-LOOP-OBJ-05` | POST_BROKEN_REST 뒤 수리·문·이동 오브젝트 상태 보존 |
