...
event_result:
  event_id: MARA2_FU
  completed: true
  set_flags: [MARA2_FU_seen]
  set_from_choice:
    mara2_name_written: selected_response == write_name
  relationship_changes:
    MARA2: {bond: 1, alert: 0}
  idempotency_key: MARA2_FU_FIRST_COMPLETE
```

`mara2_name_written`은 E3_5 보존 방식과 별도 영구 bool이다. 말로 약속하면 false, 수첩에 임시 호칭을 쓰면 true이며 두 경우 모두 MARA2_FU와 bond 변화는 최초 한 번만 적용한다.
