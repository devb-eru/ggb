...
| `17_상태변수_이벤트ID_Godot데이터구조.md` | 세부 노드, E1·F3 logical set, F2 knowledge, F3 저장·잠금, schema 12 | 완료 |
...
- 필수 사실 5종은 화면 읽기에서 `PROMISE → CONVERSION → FORCED ACTIVATION → OUTSIDE UNKNOWN → SUBJECT AUTHORITY` 순으로 제공한다.
...
| --- | --- | --- | --- |
| `D5_COMPLETION` | `D5_complete`, filter=`DISABLED`, D6 목적지 | `SAVE_D5_COMPLETE` | 전부 rollback |
...
| `F2_COMPLETION` | 필수 지식 5종, `F2_complete` | `SAVE_F2_COMPLETE` | recap에서 누락만 보수 |
...
| 상황 | 로드 기준 |
| --- | --- |
| D5 완료 뒤 종료 | D6_FREE_INSPECTION, filter DISABLED |
