...
| `FINAL_SLEEP_LOCK` | F3 진입부터 선택 회피용 수면 차단 | F3 조사·EDC 취소 뒤에도 유지 |
...
flowchart TD
    ...
    D5C --> D5R{"build_flavor"}
    D5R -- "demo" --> DEMOEND["DEMO_END_SCREEN<br/>데모 종료·승계 안내"]
    D5R -- "full" --> D6F["D6_FREE_INSPECTION<br/>마지막 파열 조사"]
    D6F --> D6R{"휴식 경로"}
    ...
    F2 --> F3["F3 장치 3종 조사"]
    F3 --> EDC["EDC 최종 선택"]
    EDC --> COMMIT["EDC_FINAL_DECISION<br/>원자 커밋·SAVE_ENDING_BRANCH"]
    COMMIT --> ALL{"all_servants_complete?"}
    ALL -- "예" --> CER["ED_ALL_CEREMONY<br/>비차단 서명"]
    ALL -- "아니오" --> HANDOFF["선택한 EDR·EDS ENTRY"]
    CER --> HANDOFF
```
...
| 사건 | 첫 플레이 | 반복·재개 | 비고 |
| --- | ---: | ---: | --- |
| D5 | 본편 2~3분 / 데모 스팅어 60~90초 | 15~30초 재개 | 같은 완료 상태, 프레젠테이션만 분리 |
...
| EDC·엔딩 인계 | 1~3분 | 커밋 전 취소 가능 | ALL 의식은 조건부 추가 |
...
| `SAVE_D5_COMPLETE` | 필터 DISABLED | demo: `DEMO_END_SCREEN`; full·승인된 import: `D6_FREE_INSPECTION` |
...
| `SAVE_F3_COMPLETE` | 장치 3종 조사 완료·진입 시 수면 잠금 유지 | EDC 전 중립 지점 |
...
| 프레젠테이션 | full: `full_transition` 2~3분 / demo: `demo_stinger` 60~90초 |
| 다음 | demo: `DEMO_END_SCREEN` / full: `D6_FREE_INSPECTION` |
...
→ D5_SLEEP_BROADCAST
→ D5_COMPLETION
→ build_flavor=demo: DEMO_END_SCREEN
→ build_flavor=full: D6_FREE_INSPECTION
```

`demo_stinger`는 `D5_HEART_STABLE`, 필터 박리, 다섯 아바타 서명 분리와 `D5_SLEEP_BROADCAST`의 핵심 비트만 유지한다. 자유 시선 선택과 후속 D6 조사는 제거하지만 `D5_COMPLETION`의 상태 쓰기와 접근성 대체는 본편과 같다. 본편은 `full_transition`으로 아래 전체 장면을 2~3분 재생한다.
...
event_completion:
  ...
  route_by_build_flavor:
    demo: DEMO_END_SCREEN
    full: D6_FREE_INSPECTION
```
...
| 완료 뒤, demo | `DEMO_END_SCREEN`; D6 registry 접근 불가 |
| 완료 뒤, full | `D6_FREE_INSPECTION` |
...
완료: {completed_relation_count} / 5
미완료: {remaining_relation_list}
남은 예상 시간: {remaining_minutes_min}~{remaining_minutes_max}분
연구원 기록: {researcher_record_count} / 5

[계속 조사한다] [기록을 정리한다]
```

placeholder는 문서 09와 현지화 계약의 의미 이름을 그대로 사용한다. `remaining_relation_list`의 각 항목은 색만 쓰지 않고 `문양 + 화면명`으로 출력한다. 남은 사건이 없으면 목록과 시간 범위를 각각 `없음`, `남은 선택 사건 없음`으로 치환하며 숫자 `0~0분`을 노출하지 않는다.
...
| 출력 | `F2_complete`, 필수 사실 6종 verified |
| 다음 | F3_ENTRY |
...
8. 연구원 해방 초안은 현재 실행 가능한 선택지가 아니다. 이론적 미래 가능성과 본편 시점의 실행 불가능성을 같은 화면에서 고지한다.
...
| --- | --- | --- | --- |
| 아버지와 동의 | 마라 2·에드가 | 미래 육체 약속과 실제 생체 신경 코어·인격 프로세스 결합 전환 | 마라 1 |
| 강제 기상 | 에드가 | 아버지 사망 뒤 외로움·원망·보호 욕구로 주인공을 강제 접속 | 루카 |
| 외부 상태 | 이리스·루카 | 외부 생존 가능성은 불명확하며 안전 보장 불가 | 에드가 |
| 연구원 해방 | 마라 2 | 원래 신체와 핵심 이전 모듈이 없고, 현재 분리는 인격 소실 위험 때문에 실행 불가 | 루카·마라 1 |
...
관계 미완료 경로에서도 여섯 knowledge는 `verified`가 된다. 감정 해석과 개인적 세부만 제한된다.

`KN_F2_RELEASE_NOT_CURRENTLY_EXECUTABLE`의 필수 표시 내용:

```text
현재 상태: 생체 신경 코어와 인격 프로세스가 결합된 한 인격
현재 시설: 호환 신체 없음 / 배양·접속 모듈 없음
불완전 분리: 기억 파편화·신경 손상·현재 인격 소실 위험
현실 분기 기본 절차: 저전력 보존
미래 복구 가능성: 외부 기반 발견 여부에 따라 미확정
```

‘실행 불가’는 영원히 불가능하다는 선언이 아니라, EDC에서 선택할 수 있는 즉시 해결책이 아니라는 뜻이다.
...
- 마라 2는 해방 초안과 함께 `배양·접속 모듈: 생명 유지 전용으로 분해` 원본 이력을 보여 준다.
- 체크섬은 의식의 백업본이 아니라 현재 인격의 손상 여부를 확인하는 자료라고 정정한다.
- 외부 산업 기반을 찾는 미래 가능성은 부정하지 않지만 본편 시점에 즉시 실행할 제3 선택은 없다고 명시한다.
...
이리스: 외부 안전 불확실
마라 1: 기록 삭제와 동의 은폐
마라 2 또는 익명 인덱스: 결합 인격 전환과 현재 해방 실행 불가
```
...
| 실패 | 없음 |
| 관계 변화 | 없음 |
| 진입 출력 | `final_sleep_lock=true` |
| 완료 출력 | `F3_complete`, EDC 활성, 수동 저장 지점 |

### 13.2 공간 중립 배치

F3 진입과 동시에 `final_sleep_lock=true`를 설정한다. 코어실에서 수면으로 E구간에 돌아가는 우회는 허용하지 않지만, F3 조사와 EDC 취소·재진입, 저장·불러오기는 계속 허용한다.
...
- 기상 커밋 시 다섯 `RESIDENT` 인격은 자동 저전력 보존으로 전환된다.

불확실한 정보:

- 장기 생존 가능성.
- 시설 밖의 인간 생존자 존재.
- 외부 산업 기반을 찾아 이전 장치를 다시 만들 수 있는지 여부.
...
```text
기상 절차
확정: 외부 신체 연결 / 현재 생존 신호 / 다섯 인격 저전력 보존
불확정: 외부 장기 생존 / 귀환·미래 이전 가능성
```
...
- 다섯 생체 신경 코어는 활동 상태를 유지하지만 현재 시설에서 새 신체로 옮길 수는 없다.
...
| 항목 | 현실 기상 | 안정화 잔류 |
| --- | --- | --- |
| 얻는 것 | 외부 현실과 물리적 행동 가능성 | 익숙한 환경과 다섯 인격의 즉시 지속 |
| 감수하는 것 | 신체·환경 위험, 보존 설비 유지, 즉시 대화 단절 | 거짓 세계임을 아는 삶, 고전력 시설과 외부 신체 자원 소비 |
| 확실성 | 외부 신체 연결, 다섯 인격 저전력 보존 | 현재 기억·관계·의식 활동 유지 |
| 불확실성 | 장기 생존·타인·미래 이전 기반 | 시설 수명·외부 신체 균형·장기 심리 변화 |
...
- F3 진입 시 `final_sleep_lock=true`로 세계 내 수면만 비활성화.
...
- 확정하면 두 값을 `ending_run.branch_id`, `branch_committed=true`와 함께 원자 저장하고 `SAVE_ENDING_BRANCH`를 생성한다.
- 커밋 전 강제 종료는 전체 롤백, 커밋 후 강제 종료는 확정 분기 전체 복구만 허용한다.
- 관계 완료 수와 연구원 기록 수는 EDC 버튼의 표시·활성·기본 포커스에 사용하지 않는다.
...
```mermaid
flowchart TD
    F3["F3_COMPLETION<br/>SAVE_F3_COMPLETE"] --> EDC["EDC"]
    EDC -- "취소" --> F3
    EDC -- "현실 확정" --> CR["EDC_FINAL_DECISION<br/>branch=reality"]
    EDC -- "잔류 확정" --> CS["EDC_FINAL_DECISION<br/>branch=stay"]
    CR --> AR{"all_servants_complete?"}
    CS --> AS{"all_servants_complete?"}
    AR -- "예" --> CER_R["ED_ALL_CEREMONY 현실 변형"]
    AS -- "예" --> CER_S["ED_ALL_CEREMONY 잔류 변형"]
    AR -- "아니오" --> EDR["EDR_ENTRY"]
    AS -- "아니오" --> EDS["EDS_ENTRY"]
    CER_R --> EDR
    CER_S --> EDS
```
...
1. `ED_ALL_CEREMONY`보다 EDC 커밋이 먼저다.
2. 의식의 필기 입력은 이미 확정된 선택의 서명이며 엔딩 허가·성공 조건이 아니다.
3. `all_servants_complete=false`여도 선택한 엔딩 ENTRY로 즉시 진행한다.
4. 의식 중 종료하면 `SAVE_ENDING_BRANCH`에서 확정 분기와 첫 미완료 인증을 복구한다.
5. EDR·EDS ENTRY 이후의 필수·선택 오브젝트, 노드 저장, 메타 커밋은 문서 14·17이 소유한다.

```yaml
ending_handoff:
  source_event: EDC_FINAL_DECISION
  required_committed_fields:
    - final_decision
    - final_choice_relation
    - ending_run.branch_committed
    - ending_run.branch_id
  required_save_point: SAVE_ENDING_BRANCH
  conditional_event:
    event_id: ED_ALL_CEREMONY
    guard: all_servants_complete
...
| F3 진입 뒤 수면으로 선택 회피 | FINAL_SLEEP_LOCK, 저장·불러오기는 허용 |
| ALL 완료가 새 엔딩 게이트가 됨 | EDC 커밋 뒤 의식 배치, `required_for_ending_success=false` |
| EDC 커밋 중 부분 저장 | `EDC_FINAL_DECISION` 원자 트랜잭션과 SAVE_ENDING_BRANCH |
...
| QA ID | 시나리오 | 기대 결과 |
| --- | --- | --- |
| `QA-13-F2-LOW` | 관계 0명, 기록 0명 | 필수 사실 6종 공개, F3 진입 |
...
| `QA-13-F3-05` | F3 진입 직후·EDC 취소 뒤 수면 시도 | FINAL_SLEEP_LOCK 안내, F3·EDC 상태 유지 |
| `QA-13-EDC-01` | EDC 커밋 각 쓰기 단계에서 종료 | 전부 롤백 또는 분기·관계·저장 지점 전부 완료 |
| `QA-13-EDC-02` | 관계 완료 0명으로 현실·잔류 각각 확정 | ALL 의식 없이 해당 ENTRY 진입 |
| `QA-13-EDC-03` | 관계 완료 5명으로 현실·잔류 각각 확정 | 커밋 뒤 ALL 의식, 의식 뒤 해당 ENTRY 진입 |
| `QA-13-EDC-04` | ALL 의식 중 종료·재개 | 선택 유지, 첫 미완료 인증부터 재개 |
...
F2 LOW에서도 필수 knowledge 6종을 얻는다.
F2·F3가 f0_provisional_intent를 레이아웃·대사 분기에 읽지 않는다.
F3 진입이 final_sleep_lock을 쓰고, F3 완료가 EDC를 연다.
...
ED_ALL_CEREMONY가 EDC 뒤에만 있고 final_decision·ending_success를 쓰지 않는다.
...
- [x] F3→EDC 원자 커밋→ALL 비차단 의식→엔딩 ENTRY 인계가 정의됨.
- [x] 감각·색상·접근성·저장 복구·QA가 정의됨.

## 20. 후속 동기화 작업

2026-07-19에 아래 후속 동기화를 완료했다. 14~17은 기존 00~12의 확정 계약과 본 문서의 상세 정의를 우선해 보완했다.

| 문서 | 후속 반영 | 상태 |
| --- | --- | --- |
| `03_전체이벤트흐름도.md` | D6 자유 조사·두 수면 경로, E1 3-of-4, F2 recap, F3 세부 흐름 | 완료 |
| `04_전체이벤트리스트_상태표.md` | D6·E1·F2·F3 사건 행, 필수 knowledge, 고유 조사 논리 집합, 상태 플래그 | 완료 |
| `05_공간구성지도_및_동선.md` | 침실 E1 4종, 서비스 척추 비상 캡슐, 코어실 F3 3종 | 완료 |
| `07_이벤트상세_02_루프_영구정보_숏컷.md` | POST_BROKEN_REST 보존, F3 진입 FINAL_SLEEP_LOCK | 완료 |
| `10_이벤트상세_05_공간잠금_해금_동선.md` | E5 뒤 MARA2_FU→EDGAR_S3→E6 인계와 가드 | 완료 |
| `11_이벤트상세_06_사용인짧은반응.md` | LUCA_S2·MARA2_FU·EDGAR_S3 발생 우선순위와 합류 | 완료 |
| `12_이벤트상세_07_사용인핵심관계.md` | J4 확정 경계와 E5 owner insert 입력 계약 | 완료 |
| `14_이벤트상세_09_엔딩.md` | F3 완료 가드, EDC 중립 포커스, 취소 시 잠금 유지 | 완료 |
| `15_이벤트상세_10_공통오브젝트반응.md` | E1 4종, E5 식탁, F3 장치 3종의 초회·반복 반응 | 완료 |
| `16_색상연출_UI_접근성규칙.md` | D5·E5·F2·F3의 비색상 대체, Windows 입력·감각 상한·선택 중립 규격 | 완료 |
| `17_상태변수_이벤트ID_Godot데이터구조.md` | 세부 노드, E1·F3 logical set, F2 knowledge, F3 저장·잠금, runtime schema 1·state model r12 | 완료 |
| `00`, `README`, `18` | 정확한 수면 상태명, 공간 분리, F0-E·F2·F3·EDC 요약 | 완료 |
| `03`~`05`, `14`~`18`, `00`, `README` | EDC 커밋 뒤 ALL 의식, EDR·EDS 에필로그, 현실 수첩·S5 동선 | 2026-07-24 완료 |

F3 잠금 시점 충돌은 `GGB-CNF-2026-0007`로 추적했다. 03·04의 선행 계약을 채택해 F3 진입 시 잠금, F3 완료 시 저장·EDC 개방으로 역할을 분리했다.

## 21. D6·E1·F3 공통 오브젝트 표현 계약

### 21.1 D6 수면 경로

| 경로 | object_id | 확인 문구의 차이 | 공통 결과 |
| --- | --- | --- | --- |
| 침실 귀환 | `OBJ_BEDROOM_BED` | 익숙한 침대가 캡슐 곡면처럼 느껴짐 | `FRACTURE_SLEEP` |
| 시설 척추 | `OBJ_EMERGENCY_REST_CAPSULE` | 장치명이 직접 노출되고 침실 투영을 거치지 않음 | `FRACTURE_SLEEP` |

두 경로는 이동 비용과 감각만 다르며 첫 사용 결과는 같다.
...
E1 이후에는 같은 오브젝트가 `POST_BROKEN_REST`를 요청한다. NORMAL_RESET·아침 스냅샷·루프 번호 증가를 호출하지 않는다.

### 21.2 E1 같은 침실의 다른 아침

| interaction_id | 부모 object_id | S3에서 드러나는 사실 |
| --- | --- | --- |
| `E1_OBJ_BED` | `OBJ_BEDROOM_BED` | 이불 아래 냉각 캡슐 곡면과 펌프 압력 |
| `E1_OBJ_WINDOW` | `OBJ_BEDROOM_WINDOW` | 새와 날씨가 움직이는 대신 같은 주기로 재생됨 |
| `E1_OBJ_MIRROR` | `OBJ_BEDROOM_MIRROR` | 반사가 주인공보다 반 박자 늦게 멈춤 |
| `E1_OBJ_CALL_CORD` | `OBJ_BEDROOM_CALL_CORD` | 호출끈이 BIO·ARCHIVE 채널로 이어짐 |

- 네 조사 중 3개가 E1 필수 이해 조건이고, 네 번째는 선택 회수다.
- 어떤 3개를 고르더라도 같은 E2에 합류한다.
- 각 조사는 `loop_state.e1_unique_interactions` Set에 interaction_id를 한 번만 기록한다.
- 오브젝트 반응은 E1 진행 카운트만 기록하며 관계값·사용인 의도를 추론하지 않는다.
- 네 번째 조사는 E2 이후 침실 재방문에서도 회수할 수 있다.

### 21.3 F3 세 장치

| interaction_id | object_id | 보여 주는 가능성 | 쓰지 않는 값 |
| --- | --- | --- | --- |
| `F3_OBJ_WAKE_DEVICE` | `OBJ_CORE_WAKE_DEVICE` | 현실 신체 기상 절차·예상 위험 | `final_decision` |
| `F3_OBJ_LOOP_STABILIZER` | `OBJ_CORE_LOOP_STABILIZER` | 기억을 유지한 안정화 루프의 조건과 한계 | `final_decision` |
| `F3_OBJ_NOTEBOOK` | `OBJ_CORE_NOTEBOOK_TERMINAL` | SUBJECT 현재 권한·과거 기록과 현재 선택의 분리 | `final_decision` |

세 장치는 모두 `loop_state.f3_unique_interactions`에 들어가야 EDC가 열린다. 조사 순서는 자유이며 첫 포커스, 버튼 크기, 색 강조는 동일하다.

### 21.4 결산 표현

- D6는 “잠을 자면 다시 돌아온다”는 기대를 깨되 실패 메시지를 쓰지 않는다.
- E1은 고딕 사물이 사라진 것이 아니라 같은 표면 아래 시설 기능이 드러났음을 보여 준다.
- F3는 현실과 잔류를 용기·겁, 진실·거짓의 도덕 축으로 배치하지 않는다.
- 관계 완료 인물의 짧은 음성·문양은 장치 설명 뒤 overlay로만 붙고 선택 가능성이나 기본 포커스를 바꾸지 않는다.
...
fracture_state:
  broken_reset_triggered: true
loop_state:
  e1_unique_interactions: [E1_OBJ_BED, E1_OBJ_WINDOW, E1_OBJ_MIRROR]
  f3_unique_interactions: []
```

시스템 확인창은 저장 데이터에 직렬화하지 않는다. 확인 도중 저장·중단하면 확인 전 상태로 복구한다. F3 조사 완료 직후에는 `f3_unique_interactions` Set을 먼저 저장하고 애니메이션을 재생한다.

### 21.6 전환 QA

| QA ID | 검증 |
| --- | --- |
| `QA-TRANS-OBJ-01` | 침대·비상 캡슐 첫 사용 결과가 같은 E1 |
| `QA-TRANS-OBJ-02` | E1 뒤 두 휴식 경로 모두 POST_BROKEN_REST |
| `QA-TRANS-OBJ-03` | E1 임의 3-of-4 조합 4종이 모두 E2 도달 |
| `QA-TRANS-OBJ-04` | F3 장치 조사 순서 6종이 모두 같은 EDC 개방 |
| `QA-TRANS-OBJ-05` | 관계 상태가 F3 포커스·버튼·필수 조사 수를 바꾸지 않음 |

## 22. 문서 16 파열·결산 Windows 연출 동기화

### 22.1 D5 감각 상한

| 효과 | 기본 표현 | 상한 | 감소 설정 |
| --- | --- | --- | --- |
| 화면 흔들림 | 태엽 심장 동기 직후 1회 | 1080p 기준 4 논리 px·0.4초 | 제거 |
| 색 잔상 | 아바타와 데이터 서명 분리 | 2~8 논리 px | 정적 이중 윤곽 |
| 프레임 지연 | 말·몸·그림자의 시차 | 1~3프레임 | 타임코드 비교 |
| 국소 대비 전환 | 위장 필터 표면 박리 | 초당 2회 미만·1초 이내 | 점진 마스크 |
| 강제 줌 | 태엽 심장→사용인→저택 골격 | 10% 이내·0.6초 이상 | 고정 프레임+대상 윤곽 |
| 입력 잠금 | `SYNC COMPLETE` 확인 구간 | 초회 3초 이내 | 초회 후 건너뛰기 |

- 전체 화면 백색 섬광과 빠른 적흑 반전을 사용하지 않는다.
- D5 HOLD 중에도 `Esc` 설정, 대사 로그, 일시정지를 허용한다.
- `SYNC COMPLETE`를 파열보다 먼저 텍스트로 제시해 실패로 오인하지 않게 한다.
- 감각을 모두 줄여도 `위장 필터 해제 성공 → 정상 리셋 불가 → 새 상태로 이동`의 세 의미를 텍스트로 전달한다.

### 22.2 S3 인물 표현

| 인물 | 아바타에서 먼저 남는 것 | 몸 밖 데이터 서명 | 비색상 식별 |
| --- | --- | --- | --- |
| 에드가 | 뿔·꼬리·레이피어의 수직 실루엣 | 남색 잠금선 | LOCK 라벨·낮은 시계음 자막 |
| 마라 1 | 여우 꼬리·스패너 작업 자세 | 주황 대각 정비 경로 | MAINT 라벨·닦임 방향 |
| 루카 | 둥근 쥐 귀·모은 손 | 검정+연두 이중 맥박 | BIO 라벨·두 파형 시간차 |
| 이리스 | 관절형 플라스틱 날개 | 흰색+연노랑 방사선 | CLIMATE 라벨·센서막 도식 |
| 마라 2 | 뾰족한 귀·망토형 날개 | 보라 이중 액자 | ARCHIVE 라벨·체크섬 구간 수 |

파열은 대표색을 제거해 “진짜 기계 외형”만 남기는 장면이 아니다. 고딕 아바타와 시설 기능, 인간 목소리와 데이터 서명을 동시에 보여 주어 어느 한쪽도 가짜 껍데기로 단순화하지 않는다.

### 22.3 E5·F2 Windows 정보 표시

- E5 결산 좌석은 키보드 `Tab`으로 이름표→소품→현재 상태 순서로 순회한다.
- UI·글자 200%에서는 다섯 좌석을 한 화면에 축소하지 않고 가로 카드 또는 세로 목록으로 전환한다.
- 색 제거 시 완료 여부는 착석, 문양 연결선, `완료/미완료` 상태명으로 표시한다.
- F2의 현재 아바타와 연구원 기록은 `현재:`·`기록:` 화자 헤더를 사용한다.
- 필수 사실 6종은 화면 읽기에서 `PROMISE → CONTINUOUS CONVERSION → RELEASE UNAVAILABLE NOW → FORCED ACTIVATION → OUTSIDE UNKNOWN → SUBJECT AUTHORITY` 순으로 제공한다.
- 관계·질문 생략 뒤 `F2_MANDATORY_RECAP`은 누락 사실만 읽고 이미 본 문장을 반복하지 않는다.
- 창 포커스 상실 시 자동 대사를 멈추고 현재 화자·문장·질문 위치를 보존한다.

### 22.4 F3 입력·포커스

```text
F3_ENTRY
→ 중앙 SUBJECT 수첩대 focus
→ 세 장치 자유 조사
→ f3_unique_interactions 3종
→ SAVE_F3_COMPLETE
→ EDC
```

- 마우스 hover와 키보드 focus가 달라도 조사 입력은 현재 사용 입력 장치의 명시적 확정만 소비한다.
- F3 진입 직전 포인터가 기상·잔류 장치 위에 있어도 중앙 수첩대로 시작한다.
- 각 장치는 44×44 논리 px 이상, 큰 타깃 56×56 논리 px 이상을 보장한다.
- 장치 설명은 `확실한 정보`, `불확실한 정보`, `실행 시 바뀌는 것`, `아직 모르는 것`의 같은 구조를 사용한다.
- F0 임시 의향, 마지막 조사 장치, 관계 등급은 포커스·카메라·음향·버튼 순서를 바꾸지 않는다.
- `Alt+Tab` 복귀 뒤 마지막 완료 조사 Set은 유지하되 미확정 조사·EDC 선택은 자동 완료하지 않는다.

### 22.5 복합 QA

| QA ID | 조건 | 기대 |
| --- | --- | --- |
| `QA-TRANS-WIN-01` | 1280×720·UI 200% | D5 상태문·E5 좌석·F2 사실·F3 장치 접근 가능 |
| `QA-TRANS-WIN-02` | D5 중 `Alt+Tab` | 입력 자동 제출 없이 HOLD·대사 위치 유지 |
| `QA-TRANS-WIN-03` | 키보드 전용 | D6→E1→E5→F2→F3 완료 |
| `QA-TRANS-A11Y-01` | 색 제거+음량 0+글리치 0 | D5 성공·5인 출처·F2 필수 사실 이해 |
| `QA-TRANS-A11Y-02` | 모션 최소 | D5·S3 의미 유지, 흔들림·강제 줌 없음 |
| `QA-TRANS-NEUTRAL-01` | F0 의향·관계·조사 순서 전 조합 | F3 첫 포커스와 EDC 개방 상태 동일 |

## 23. 문서 17 저장·트랜잭션 동기화

### 23.1 상태 루트 소유권

| 본 문서 상태 | 저장 경로 | 수명 | writer |
| --- | --- | --- | --- |
| `D5_complete` | `meta_progress.flags` | 슬롯 영구 | D5 완료 group |
| `camouflage_filter_state` | `fracture_state` | 슬롯 영구 | D5·BROKEN_RESET |
| `broken_reset_triggered` | `fracture_state` | 슬롯 영구 | BROKEN_RESET 전용 |
| `e1_unique_interactions` | `loop_state` | S3 구간 | E1 controller |
| `relationship_hub_open` | `fracture_state` | 슬롯 영구 | E2 완료 |
| E3 lifecycle·outcome | `meta_progress.event_history` | 슬롯 영구 | servant core event |
| E3 relation·record | `meta_progress.servant_states`·records | 슬롯 영구 | 같은 완료 group |
| `journal_stage=4` | `meta_progress` | 슬롯 영구 | J4 확정 |
| `e5_locked_in` | `fracture_state` | 슬롯 영구 | E5 완료 |
| `f3_unique_interactions` | `loop_state` | F3 실행·재개 | F3 controller |
| `final_sleep_lock` | `fracture_state` | 슬롯 영구 | F3_ENTRY |
| `final_decision` | `meta_progress` | 슬롯 영구 | EDC 전용 |
| `ending_run` | 진행 슬롯 | 엔딩 실행 | EndingRouter |

scene·대사·UI는 위 값을 직접 쓰지 않는다. 사건 controller가 `StateWriter`에 선언된 write를 요청하고 revision 확정 뒤 연출을 갱신한다.

### 23.2 원자 그룹

| atomic group | 함께 쓰는 값 | 저장 지점 | 부분 실패 |
| --- | --- | --- | --- |
| `D5_COMPLETION` | `D5_complete`, filter=`DISABLED`, `content_boundary_id=SAVE_D5_COMPLETE`, flavor별 재개 라우트 | `SAVE_D5_COMPLETE` | 전부 rollback |
| `BROKEN_RESET_COMPLETION` | S3 생성, filter=`BROKEN`, broken flag, sleep complete, SYS_SYNC | `SAVE_BROKEN_RESET_COMPLETE` | S3 중복 생성 금지 |
| `E3_X_COMPLETION` | 완료·REC·사용인 상태·bond·alert·history | 사건 완료 자동 저장 | 관계 delta 1회 |
| `J4_CONFIRMATION` | journal stage 4, 미완료 E3 superseded, checkpoint 정리 | `SAVE_J4_COMPLETE` | 확인 전 상태 유지 |
| `E5_COMPLETION` | `e5_locked_in`, 결산 snapshot, 후속 queue | `SAVE_E5_COMPLETE` | E_HUB 복귀 |
| `F2_COMPLETION` | 필수 지식 6종, `F2_complete` | `SAVE_F2_COMPLETE` | recap에서 누락만 보수 |
| `F3_COMPLETION` | 고유 조사 3종, `F3_complete`, EDC 개방 | `SAVE_F3_COMPLETE` | 선택 미확정 유지 |
| `EDC_FINAL_DECISION` | final decision·choice relation·branch commit | `SAVE_ENDING_BRANCH` | 전부 rollback 또는 완료 |

`F3_ENTRY`의 `final_sleep_lock=true`는 F3 완료와 분리한다. F3 완료 transaction이 실패해도 잠금은 유지되고 안전한 F3 조사 지점으로 돌아간다.
...
| 상황 | 로드 기준 |
| --- | --- |
| D5 완료 뒤 종료, demo | `DEMO_END_SCREEN`, filter DISABLED, D6 registry 접근 불가 |
| D5 완료 뒤 종료, full | `D6_FREE_INSPECTION`, filter DISABLED |
| 승인된 demo 저장 import 뒤 종료 | 원본 demo 슬롯 보존, 복제된 full 슬롯만 `D6_FREE_INSPECTION` |
| FRACTURE_SLEEP 확정 뒤 종료 | 같은 수면 경로, 미완료 BROKEN_RESET |
| BROKEN_RESET 완료 뒤 종료 | E1_ENTRY, S3 재생성 금지 |
| E3 도중 종료 | 첫 미검증 node |
| J4 확인 취소 | E_HUB, 어떤 상태도 쓰지 않음 |
| J4 commit 뒤 종료 | E3_4 또는 E3_4M |
| F3 진입 뒤 종료 | final sleep lock 유지·F3 안전 조사 |
| F3 완료 뒤 종료 | F3_COMPLETION·EDC 재개방 |
| EDC commit 뒤 종료 | 확정 분기 ENTRY |

tmp 파일은 checksum 검증 전 main을 대체하지 않는다. main 손상 시 backup의 마지막 완전한 transaction을 사용하며 둘 다 손상되면 자동 추정 대신 복구 화면을 연다.

### 23.4 논리 집합

`e1_unique_interactions`, `f3_unique_interactions`, `ending_run.completed_nodes`는 논리 집합이다.

- Godot 4.7 런타임은 중복 없는 `Array[StringName]`.
- JSON은 사전순으로 정렬한 문자열 배열.
- 같은 오브젝트 반복은 고유 수를 늘리지 않음.
- `interaction_nodes`처럼 순서가 의미인 배열은 정렬 금지.
- legacy 중복값은 제거하되 완료 보상을 다시 적용하지 않음.
...
- `ResetCoordinator`가 SYS_COMMIT→SYS_MEMORY→NORMAL_RESET·BROKEN_RESET→ROUTE를 조정한다.
- `EndingRouter`는 EDC 이후만 소유하며 F0 의향이나 관계값을 수정하지 않는다.
- `VisualStateResolver`는 filter·world phase·관계값을 읽지만 진행 상태를 쓰지 않는다.
- `state_committed` 뒤 장면과 UI가 새 상태를 읽는다.
- `Alt+Tab` 복귀 입력은 사건 confirm 신호로 전달하지 않는다.

### 23.6 데이터·저장 QA

| QA ID | 조건 | 기대 |
| --- | --- | --- |
| `QA-TRANS-DATA-01` | D5 atomic write 단계마다 종료 | ACTIVE 유지 또는 DISABLED 완료, 혼합 상태 없음 |
| `QA-TRANS-DATA-02` | BROKEN_RESET write 단계마다 종료 | S3 0회 또는 1회 생성 |
| `QA-TRANS-DATA-03` | E3 완료 저장을 두 번 재개 | relation delta 1회 |
| `QA-TRANS-DATA-04` | J4 commit 전·후 종료 | hub 유지 또는 확정 경로, checkpoint 모순 없음 |
| `QA-TRANS-DATA-05` | F3 3종을 다른 순서로 조사 | 같은 logical set·SAVE_F3_COMPLETE |
| `QA-TRANS-DATA-06` | EDC commit 중 종료 | 결정·관계·branch·save point 전부 rollback 또는 완료 |
| `QA-TRANS-DATA-07` | progress 손상·backup 정상 | 마지막 완전한 transaction 복구 |
| `QA-TRANS-DATA-08` | 접근성 profile 손상 | 진행·결산 상태 보존, profile 기본값 복구 |
