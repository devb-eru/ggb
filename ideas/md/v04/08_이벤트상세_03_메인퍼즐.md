...
### 2.4 E3_4 기술 정답과 관계 선택 경계

E3_4의 전체 대사·감정 선택은 `12`, 저장 계약은 `17`이 담당한다. 본 문서는 퍼즐 정답이 관계 선택에 의해 바뀌지 않는다는 경계만 고정한다.

| 기능 | 기술 정답 | 판정 근거 |
| --- | --- | --- |
| 보호 `PROTECTION` | `SYSTEM` | 안전 모듈은 보호하지만 종료 여부를 결정하지 못함 |
| 감시 `SURVEILLANCE` | `CUSTODIAN` | 관리자는 관찰·보고·접근 제한을 수행함 |
| 기억 `MEMORY` | `RESIDENT` | 기억의 원 소유권은 거주 인격에게 있음 |
| 선택 `CHOICE` | `SUBJECT` | 기상·잔류를 포함한 최종 결정은 당사자에게 있음 |
...
| `outcome_id` | 관계적 의미 | 기술 결과 |
| --- | --- | --- |
| `responsibility_recorded` | 에드가 자신의 결정을 공식 감사 기록에 남김 | `CHOICE=SUBJECT` 유지 |
| `authority_returned` | 에드가가 주인공 앞에서 선택 권한을 직접 반환 | `CHOICE=SUBJECT` 유지 |

- 권한 오배치는 해당 카드만 되돌리는 LOCAL RETRY다.
- 관계 선택은 퍼즐 성공 뒤에만 열리며 퍼즐 정답을 재검증하지 않는다.
- 두 outcome 모두 REC_EDGAR와 E3_4 완료를 제공한다.
- E3_4M은 퍼즐·관계 선택의 축약판이 아니다. `edgar_minimum_access`만 제공하며 REC_EDGAR, 관계 변화, E3_4 완료를 만들지 않는다.
- 색은 출처 확인에만 쓰며 기능·소유자 정답은 라벨과 기록 문장으로 판단한다.
...
4. 맞은 슬롯과 틀린 범주 기록.
5. 목격 사용인 판정.
6. `source_event_id=B3_B`인 실패 기록을 active로 생성·갱신.
7. 남은 자유 조사.
8. 수면 뒤 BSHORT.
...
outputs:
  ...
  create_failure_record:
    source_event_id: B3_B
    observed_result_id: B3_B_ACTUAL_RUN_FAILED
    validated_steps_from_attempt: true
    invalidated_steps_from_attempt: true
  set_loop_state:
    current_hard_failure_event_id: B3_B
...
failure_transition:
  ...
  from: active
  to: resolved
  resolved_by_event_id: B4
  clear_runtime: [current_hard_failure_event_id, shortcut_resume]
```
...
4. 목격자 판정.
5. `source_event_id=C4`인 실패 기록을 active로 생성·갱신.
6. 남은 조사 뒤 수면.
...
outputs:
  ...
  create_failure_record:
    source_event_id: C4
    observed_result_id: C4_ACTUAL_TRACE_FAILED
    validated_steps_from_attempt: true
    invalidated_steps_from_attempt: true
  set_loop_state:
    current_hard_failure_event_id: C4
...
- 연구원 정체와 생체 신경 코어·인격 프로세스 결합 진실은 공개하지 않는다.
...
outputs:
  ...
  resolve_failure_records:
    - C4
  clear_runtime: [current_hard_failure_event_id, shortcut_resume]
  next_event: C5_INFO
```

`C5`는 회로와 채널을 노출하는 결과 이벤트이고, `C5_INFO`는 그 자료를 수첩에 고정하는 정보 조사 이벤트다. `C5_INFO` 완료 시에만 `c5_info_complete`와 `color_room_entry_inspectable`을 기록하고, `M1_COLOR_ROOM_ENTRY`의 외부 렌즈·필터판 조사를 연다. J3은 이 정보 조사를 거쳐 진입한다.
...
→ 이후 축 입력 잠금
→ 이전 verified 단계 기록
→ source_event_id=D1인 실패 기록을 active로 생성·갱신
→ 남은 조사
→ 수면
→ DSHORT
```
...
outputs:
  ...
  create_failure_record:
    ...
    invalidated_steps_from_attempt: true
  set_loop_state:
    current_hard_failure_event_id: D1
...
```yaml
relationship_choice:
  event_id: E3_5_RELATIONSHIP_CHOICE
  required_input: selected_resolution
  choices:
    merged:
      set: {servants.MARA2.archive_resolution: merged}
      relationship: {bond: 2, alert: 1}
      branch_variant: E3_5_MERGED
    separated:
      set: {servants.MARA2.archive_resolution: separated}
      relationship: {bond: 1, alert: -1}
      branch_variant: E3_5_SEPARATED
  atomic_commit_group: E3_5_COMPLETION
  common_outputs:
    set_flags: [E3_5_complete]
    add_records: [REC_MARA2]
    set_servant_flags: [core_event_complete, researcher_record_acquired]
...
보존 방식을 선택하기 전에는 `E3_5_puzzle_solved` 체크포인트만 유지하며 E3_5 완료·기록·관계 변화는 커밋하지 않는다. 완료 묶음 저장에 실패하면 선택 화면으로 복귀하고 같은 transaction ID 재시도에서 관계 수치를 중복 적용하지 않는다.
...
| 노드 | 위치 | 완료 출력·다음 처리 |
| --- | --- | --- |
| `E3_5_ENTRY` | `M1_COLOR_ROOM_ENTRY`→`H0_COLOR_SEPARATION` | 사건 전환 뒤 출처 분리 시작 |
| `E3_5_SOURCE_SPLIT` | `H0_COLOR_SEPARATION` | 다섯 색·문양·파형 출처 분리 |
| `E3_5_PURPLE_OVERLAY` | `H0_COLOR_SEPARATION` | `mara2_purple_channel_separated` |
| `E3_5_CHECKSUM_GAPS` | `H0_COLOR_SEPARATION` | `mara2_checksum_gaps_found`, 아카이브 문 개방 |
| `E3_5_ARCHIVE_TRANSFER` | 두 시설 사이 | `H0_PERSONALITY_ARCHIVE` 로드 |
| `E3_5_DISTRIBUTED_BACKUP` | `H0_PERSONALITY_ARCHIVE` | `mara2_distributed_backup_found`; 검증 단계가 모두 모이면 `E3_5_puzzle_solved` 파생 |
| `E3_5_SELF_SACRIFICE_DIALOGUE` | `H0_PERSONALITY_ARCHIVE` | 자기 저장 영역 양도 사실 확인 |
| `E3_5_RELATIONSHIP_CHOICE` | `H0_PERSONALITY_ARCHIVE` | 취소 가능; 확정 시 두 보존 분기로 이동 |
| `E3_5_MERGED`·`E3_5_SEPARATED` | `H0_PERSONALITY_ARCHIVE` | 선택 결과를 `E3_5_COMPLETION`에 전달 |
| `E3_5_COMPLETION` | `H0_PERSONALITY_ARCHIVE` | 원자적 완료 묶음 커밋 |
| `E3_5_RETURN` | `H0_PERSONALITY_ARCHIVE`→중앙홀 | `E_HUB` 복귀 |

선택 확정 전 이탈은 `E_HUB`로 돌아가며 재진입 시 첫 미검증 노드에서 재개한다. `E3_5_puzzle_solved=true`이면 대화·선택 구간부터 재개한다. 선택 확정 뒤에는 `E3_5_COMPLETION` 전체가 성공하거나 선택 화면으로 롤백되며 중간 노드에 머물지 않는다.
...
| 요소 | 대체 |
| --- | --- |
| 종·진동 | 파형·리듬 자막·선택형 화면 테두리 펄스. 모션 감소 시 정적 굵기 강조 |
| 색상 | 문양·선 패턴·소유자 라벨 |
| 투명지 회전 | 90° 버튼 |
...
1. 네 기능을 오배치해도 장치가 영구 잠기지 않고 해당 카드만 재입력되는지 확인.
2. `responsibility_recorded`와 `authority_returned` 모두 `CHOICE=SUBJECT`를 유지하는지 확인.
3. 관계 선택 화면이 기술 정답·힌트·퍼즐 성공을 다시 바꾸지 않는지 확인.
4. E3_4M 경로에서 E3_4 퍼즐과 관계 선택이 실행되지 않는지 확인.
5. 색 제거·음량 0에서도 기능명·소유자 라벨·잠금선·자막으로 해결 가능한지 확인.
...
- E3_4는 `CHOICE=SUBJECT` 기술 정답과 두 관계 outcome을 분리한다.
- E3_4M은 E3_4 퍼즐·기록·관계 결과를 대체 지급하지 않는다.
...
| `11` | 퍼즐 중 짧은 사용인 반응 |
| `12` | E3_4 기술 정답·관계 outcome 분리, E3_5 관계 선택·전체 10~16분 |
| `13` | D4 성공 뒤 D5 감정 전환 |
| `16` | 퍼즐 UI·접근성 규칙 |
...
## 35. 메인 퍼즐과 공통 반응의 소유권

### 35.1 단일 입력 소유자

한 클릭은 가장 높은 우선순위의 한 이벤트만 상태를 쓴다. 공통 반응은 퍼즐 설명을 덧붙일 수 있지만 퍼즐 정답을 판정하거나 성공 플래그를 대신 기록하지 않는다.

| 퍼즐 구간 | 입력 오브젝트 | 활성 시 역할 | 상태 쓰기 소유자 |
| --- | --- | --- | --- |
| B3 | `OBJ_LIBRARY_OUTER_CLOCK` | `MAIN_PUZZLE` | B3-A·B3-B 퍼즐 컨트롤러 |
| C3·C4 | `OBJ_MIRROR_BLACK` | `MAIN_PUZZLE` | C3 혼합·C4 거울 청소 컨트롤러 |
| D1 | `OBJ_BASEMENT_AXIS_DEVICE` | `MAIN_PUZZLE` | D1 세 축 컨트롤러 |
| E3_5 | `OBJ_PERSONALITY_ARCHIVE_TERMINAL` | `MAIN_PUZZLE` | E3_5 원본 대조 컨트롤러 |
| F0 | 코어 접근로 패널 | `MAIN_PUZZLE` | F0-A~E 단계별 컨트롤러 |

비활성 시 같은 대상은 `INFO_RECORD` 또는 `AMBIENT` fallback을 사용할 수 있다. 예를 들어 기록 시계는 B3 밖에서는 시간표 단서를 말하지만, B3 활성 중에는 클릭을 종 파형 입력으로만 해석한다.

### 35.2 역할별 허용 효과

| 역할 | 허용 | 금지 |
| --- | --- | --- |
| `MAIN_PUZZLE` | 현재 단계 입력, 오답 피드백, 전용 failure·solution 커밋 | 관계 수치·최종 선택 쓰기 |
| `INFO_RECORD` | 명시된 knowledge·수첩 항목 | 퍼즐 성공·실패 판정 |
| `AMBIENT` | 세계 상태·감각·first/repeat 이력 | 모든 진행 플래그 |
| `RELATION_OVERLAY` | owner별 설명·동의 상태 표시 | 퍼즐 힌트를 정답으로 치환 |

공통 오브젝트 반응의 `forbidden_writes`에는 항상 `puzzle_result`가 들어간다. 퍼즐 컨트롤러가 성공을 원자 커밋한 뒤에만 오브젝트의 `object_state`가 결과 상태로 바뀐다.

### 35.3 퍼즐별 fallback

- `OBJ_LIBRARY_OUTER_CLOCK`: B3 실패 잠금 중에는 실패 원인과 다음 루프 정보만 제공한다.
- `OBJ_LIBRARY_RECORD_CLOCK`: B3 상태를 읽지 않고 기록 내실에서 에드가 진입 예고만 제공한다.
- `OBJ_MIRROR_BLACK`: C4 실패 뒤 코팅 경화 상태를 묘사하고, 재시도는 RESET·CSHORT 계약으로 돌린다.
- `OBJ_BASEMENT_AXIS_DEVICE`: DF 뒤 물리 잠금과 기록된 깊이를 보여 주며 현 루프 재입력을 받지 않는다.
- `OBJ_PERSONALITY_ARCHIVE_TERMINAL`: E3_5 미완료 이탈 시 단계 체크포인트를 보여 주되 관계 결과를 확정하지 않는다.
- F0 패널: 현재 단계 이전·다음 패널을 비활성화하고, F0-E 의향은 세 선택 모두 성공으로 처리한다.
...
interaction_resolution:
  ...
  permanent_info: 50
  ambient: 10
  consume_input_once: true
```

- 퍼즐 UI가 열린 동안 배경 핫스폿은 포커스 순회에서 제외한다.
- 닫기·취소는 퍼즐 상태를 저장하되 성공·실패로 기록하지 않는다.
- owner 색상·음향은 출처 식별 보조이며 정답 역할을 대신하지 않는다.
- 힌트 요청은 동일 퍼즐 컨트롤러가 처리하며 공통 오브젝트의 반복 클릭 횟수로 자동 정답을 열지 않는다.

### 35.5 퍼즐 QA

| QA ID | 검증 |
| --- | --- |
| `QA-PUZ-OBJ-01` | 다섯 퍼즐 입력 오브젝트에서 한 클릭당 상태 writer 1개 |
| `QA-PUZ-OBJ-02` | 퍼즐 비활성 fallback 조사로 success·failure 불변 |
| `QA-PUZ-OBJ-03` | 닫기·재개로 오답 횟수나 관계값이 중복 증가하지 않음 |
| `QA-PUZ-OBJ-04` | 색·음향 제거 후에도 문양·라벨·배치로 정답 도출 가능 |
| `QA-PUZ-OBJ-05` | F0-E 세 의향 모두 F1에 합류하고 EDC 선택지를 편향하지 않음 |
