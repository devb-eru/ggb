...
핵심 관계 사건의 감정이 D5 이후 처음 시작된 것처럼 보이지 않게, D5 이전 짧은 반응 다섯 개를 관계 씨앗으로 읽는다.

```text
D5 이전 관계 씨앗 0~5개
→ E2에서 기억하고 있던 태도 확인
→ E3에서 같은 질문을 더 큰 책임 문제로 확장
→ F2·엔딩에서 완료 여부와 별개로 과거 반응 회수
```

- 관계 씨앗은 E3 진입 조건이 아니다.
- 씨앗 선택의 `bond`·`alert` 변화는 유지하되 E3 기술 정답을 바꾸지 않는다.
- 씨앗 미확인 시 E3 도입은 기능 문제부터 시작하고, 확인 시 해당 사소한 물건·문장을 한 번 회상한다.
- 권장 경험은 D5 이전 씨앗 2~3개, D5 이후 핵심 관계 2~3개다. 전원 완료는 수집형 확장 경험으로 둔다.

## 3. 전체 관계 사건 흐름

```mermaid
flowchart TB
    SEED["D5 이전 관계 씨앗 0~5개<br/>대사 변형만 저장"] -. "정서 선행값" .-> E2
...
| 경로 | 목표 시간 | 용도 |
| --- | ---: | --- |
| 본선 우선 | E2 + E3 0~1명, 약 8~25분 | 관계를 최소화해도 본선 완주 보장 |
| 권장 관계 | E2 + E3 2~3명, 약 26~53분 | 서로 다른 책임 관점을 비교하는 표준 경험 |
| 전원 관계 | E2 + E3 5명, 약 55~80분 | ALL 결산과 모든 개인 후속 확인 |

첫 플레이 안내는 “2~3명 권장”을 숫자 퀘스트로 띄우지 않는다. J4 확인 화면에서 남은 시간과 미완료 인물만 투명하게 알려 플레이어가 스스로 정한다.
...
`E2_INTRO_SEED_RECALL`은 확인한 관계 씨앗을 8~15초의 무대 반응으로 회수한다. 대사를 다섯 번 연속 재생하지 않는다.

| 관계 씨앗 확인 | 짧은 회수 동작 | E3 도입 회수 문장 |
| --- | --- | --- |
| `EDGAR_S1` | 에드가가 찻잔 흠집을 가리려다 손을 멈춤 | “그 꿈을 귀하만 기억한 것은 아닙니다.” |
| `MARA1_S1` | 마라 1이 말없이 열여덟 걸음 표식을 스패너로 가리킴 | “빠른 길까지 세어 둔 이유를 이제 보여드림다.” |
| `LUCA_S1` | 루카가 같은 온도의 잔 대신 현재 손목 온도를 먼저 확인 | “이번에는... 아가씨 기준부터 볼게요.” |
| `IRIS_S1` | 이리스의 날개에 젖지 않는 빗소리 자막이 한 번 뜸 | “비가 아니라고 했던 말, 기억하고 있어요.” |
| `MARA2_S1` | 마라 2가 자기 이름표를 주인공 쪽으로 돌려 둠 | “이번 기록에는 내 이름부터 틀리지 마!” |

- 0개면 본 비트 없이 다음 보고로 넘어간다.
- 1~5개면 동작을 3초 이내 동시 몽타주로 묶고, E3 도입에서 해당 인물만 한 문장을 말한다.
- 이 회수는 새 관계값, 기록, 지식을 쓰지 않는다.
...
| “왜 모두 기억하고 있어요?” | 사용인의 생체 신경 코어·인격 프로세스와 잔류 기억은 물리 RESET 대상이 아니었음 | 에드가가 이를 오래 숨겼음을 인정 |
...
D5가 고딕 위장 필터와 정상 RESET을 정지시키면서, 기존에 매일 세계를 재생성하던 전력이 일시적으로 남는다. E2에서 에드가는 이 전력을 파열 구역의 진행을 멈추는 완충막에 배분한다. 그 결과 E_HUB가 열린 동안 저택은 불안정한 S3 외형을 유지하지만 실시간으로 더 붕괴하지 않는다.

설계 규칙:

- 실제 시간 제한, 카운트다운, 관계 사건 수 제한을 두지 않는다.
- E3 도중 긴급 경보가 반복해서 플레이어를 재촉하지 않는다.
- 중앙홀 전력계는 `완충 유지 중`이라고만 표시하고 남은 분·퍼센트를 보여 주지 않는다.
- J4 진행을 확정하면 완충 전력을 코어 경로와 마지막 정상 저녁에 재배분한다. 이때 미완료 E3가 `superseded`된다.
- J4 확인을 취소하면 완충 상태와 모든 E3 체크포인트가 그대로 유지된다.
- 이 시간창은 서사적 호흡을 설명하는 장치이며 플레이어를 속이는 가짜 제한이 아니다.

런타임에서는 별도 저장 변수를 만들지 않고 아래 파생값을 사용한다.

```text
e_hub_stability_window_active
= fracture_state.relationship_hub_open
AND fracture_state.e5_locked_in == false
AND meta_progress.F3_complete == false
```

S3의 소리와 시각은 계속 불안하지만, 플레이어가 관계 대화를 오래 듣는 동안 세계가 기다려 주는 이유가 생긴다.
...
- 연구원들은 미래의 새 육체를 약속받았으나 원래 신체 소실과 생체 신경 코어·인격 프로세스 결합 전환을 고지받지 못했다.
...
2. 아버지는 미래의 새 신체와 각성 가능성을 약속했지만, 원래 신체 소실과 생체 신경 코어·인격 프로세스 결합 방식을 완전하게 밝히지 않았다.
...
| `16_색상연출_UI_접근성규칙.md` | 5인 실루엣·대표색, Windows 관계 UI, 동의 핫스폿·선택 중립·복합 접근성 | 완료 |
| `17_상태변수_이벤트ID_Godot데이터구조.md` | event_history, E3_1~4 원자 완료, E3_4M 예외를 유지한 runtime schema 1·state model r12 | 완료 |
...
## 21. 사용인 신체 오브젝트 동의·관계 오버레이

### 21.1 목적과 경계

사용인의 기계 외형은 인격 데이터와 신체 자율성이 동시에 드러나는 장치다. 조사 가능한 포트가 보이더라도 주인공이 임의로 만지는 구조로 만들지 않는다.

- 신체 조사에는 `consent_scope`가 필요하다.
- 동의는 해당 장면·부위·목적으로 한정되며 다른 사용인이나 다음 장면에 전용되지 않는다.
- 거절·중단은 관계 감점이나 메인 진행 실패를 만들지 않는다.
- 핵심 수리는 전용 관계 이벤트가 소유하고, 공통 반응은 설명·감각·현재 outcome만 읽는다.
- 장난스러운 인물도 신체 접촉 동의를 농담으로 무효화하지 않는다.

### 21.2 대상

| owner_id | servant object_id | 조사 부위 | 활성 장면 |
| --- | --- | --- | --- |
| `EDGAR` | `SERVANT_OBJ_EDGAR_LOCK_PORT` | 장갑 아래 잠금 인터페이스 | E3_4 진단·복원 동의 뒤 |
| `MARA1` | `SERVANT_OBJ_MARA1_MAINTENANCE_PORT` | 팔의 정비 브러시 포트 | E3_1 수리 준비 뒤 |
| `LUCA` | `SERVANT_OBJ_LUCA_BIO_PORT` | 손목 생체 포트·쥐 귀 신호부 | E3_3 안정화 동의 뒤 |
| `IRIS` | `SERVANT_OBJ_IRIS_SENSOR_MEMBRANE` | 플라스틱 날개 센서막 | E3_2 교정 동의 뒤 |
| `MARA2` | `SERVANT_OBJ_MARA2_ARCHIVE_PORT` | 목깃 액자핀·기록 포트 | E3_5 대조·보존 선택 중 |
...
consent_scope:
  owner_id: LUCA
  object_id: SERVANT_OBJ_LUCA_BIO_PORT
  event_id: E3_3
  purpose: bio_signal_stabilization
  granted: true
  valid_until: event_exit
  reversible: true
  allowed_actions:
    - inspect
    - connect_probe
...
- 유효하지 않은 신체 핫스폿은 숨긴다. 잠금 아이콘으로 호기심을 자극하지 않는다.
- 플레이어가 중단하면 도구를 분리하고 대화 선택으로 복귀한다.
- 이벤트를 나갔다 돌아오면 다시 동의를 확인한다.
- POST_BROKEN_REST는 수리 결과를 보존하지만 과거의 접촉 동의를 보존하지 않는다.

### 21.4 관계 outcome 오버레이

오브젝트 반응은 다음 순서로 표현을 합성한다.
...
→ bond band
→ alert band
→ world phase sensory layer
```

| 상태 | 표현 변화 | 진행 효과 |
| --- | --- | --- |
| 핵심 이벤트 미완료 | 기능명·손상 정도만 표시 | 없음 |
| 완료·낮은 bond | 필요한 사실만 짧게 설명 | 없음 |
| 완료·중간 bond | 연구원 시절 습관이나 소지품 기억 1줄 | 없음 |
| 완료·높은 bond | 주인공에게 맡긴 이유·두려움 1줄 | 없음 |
| 높은 alert | 접촉 전 확인 문구 강화, 물리적 위협 없음 | 없음 |

관계 오버레이는 이미 확정된 outcome을 다시 보여 주는 읽기 전용 표현이다. `bond`·`alert`·기록 획득·엔딩 등급을 변경하지 않는다.

### 21.5 인물별 감각 회수

- 에드가: 레이피어 손잡이와 같은 수직 잠금선, 낮은 시계음. 높은 관계에서는 그가 직접 장갑을 벗어 인터페이스를 보여 준다.
- 마라 1: 스패너 자국과 대각선 정비 홈. 농담 뒤 손의 미세한 떨림을 남겨 손상을 가볍게 만들지 않는다.
- 루카: 검정 외장과 연두 신호부, 이중 맥박. 말줄임표가 길어져도 필요한 안전 절차는 끝까지 말한다.
- 이리스: 옅은 노랑 센서막과 플라스틱 날개의 마찰음. 관계에 따라 적대 고백의 직접성이 달라도 강제 접촉은 허용하지 않는다.
- 마라 2: 보라 이중 윤곽과 빠른 3음. 자기 이름 확인 습관은 기록 손상의 결과로 드러나며 희화화하지 않는다.

### 21.6 접근성

- 색을 제거하면 부위 명칭, 문양, 포트 라벨을 함께 표시한다.
- 음량 0에서는 시계음·맥박·3음을 자막 패턴으로 바꾼다.
- 정밀 드래그 대신 `연결`, `확인`, `분리` 단계 입력을 제공한다.
- 신체 확대 화면은 언제든 같은 버튼으로 닫을 수 있고 카메라 흔들림을 사용하지 않는다.

### 21.7 관계 QA

| QA ID | 검증 |
| --- | --- |
| `QA-REL-OBJ-01` | 동의 전 servant object 핫스폿 비노출 |
| `QA-REL-OBJ-02` | 이벤트 이탈·재진입 때 동의 재확인 |
| `QA-REL-OBJ-03` | 거절·중단으로 bond·alert·메인 진행 불변 |
| `QA-REL-OBJ-04` | 다섯 outcome overlay가 읽기 전용으로 동작 |
| `QA-REL-OBJ-05` | POST_BROKEN_REST는 수리 결과만 보존하고 consent_scope는 폐기 |

## 22. 문서 16 관계 UI·캐릭터 표현 동기화

### 22.1 E3별 시각·비색상 표현

| 사건 | 아바타 대표 표현 | 데이터 서명 표현 | 선택 직전 중립 상태 | 감각 감소 대체 |
| --- | --- | --- | --- | --- |
| E3_1 마라 1 | 큰 꼬리가 작업 공간을 감쌈, 스패너를 양손으로 잡음 | 주황 대각 정비선 | 원문 보존·식별자 보호를 같은 면적에 배치 | 대각선 번호·삭제 범위 텍스트 |
| E3_2 이리스 | 플라스틱 날개가 센서막을 가렸다가 직접 펼침 | 흰색+연노랑 방사선 | 외부 진실·투영 유지의 정보량과 확인 횟수 동일 | 계절명·센서값·날개 관절 도식 |
| E3_3 루카 | 움츠린 자세에서 의료 조작 중 손만 안정 | 검정+연두 이중 맥박 | 전체 공개·안정 우선에 도덕 아이콘 없음 | 두 파형 시간차·절차 단계 |
| E3_4 에드가 | 레이피어를 수직 권한 홈에서 빼 손잡이를 돌림 | 남색 잠금선 | 책임 기록·권한 반환 모두 `CHOICE=SUBJECT` 뒤 표시 | 권한 슬롯명·레이피어 방향 도식 |
| E3_5 마라 2 | 망토형 날개 안쪽 체크섬과 이름표를 펼침 | 보라 이중 액자 | 병합·분리 모두 손실·보존 정보를 같은 순서로 제시 | 구간 수·원본/복사본 라벨·3음 자막 |

- 관계 결과는 색의 밝기·채도·깨끗함으로 평가하지 않는다.
- 낮은 관계에서도 인물의 이름, 대표색, 종족 특징, 소품을 제거하지 않는다.
- 높은 관계는 말·입 모양·소품 전달·데이터 잔상의 동기화로 표현한다.
- 높은 alert는 붉은 경고가 아니라 닫힌 자세, 접촉 전 재확인, 거리 유지로 표현한다.

### 22.2 Windows 선택·재진입

- 관계 UI는 마우스와 키보드로 완전히 조작할 수 있다.
- `Tab` 순서는 현재 대화 로그 → 선택지 → 기록 다시 보기 → 힌트 → 사건 나가기다.
- 관계 선택 화면 진입 시 결과 선택지에 기본 포커스를 두지 않고 `기록을 다시 본다` 또는 중립 설명 영역에 둔다.
- hover만으로 선택 설명을 열 수 있지만 클릭·`Enter` 전에는 outcome을 확정하지 않는다.
- `Esc`는 선택 확정 전 사건 이탈 확인을 열고, 이미 확정한 outcome을 되돌리지 않는다.
- `Alt+Tab`이나 창 포커스 상실 시 자동 진행과 선택 타이머를 멈춘다. 본 사건에는 선택 시간 제한을 두지 않는다.
- UI·글자 200%에서는 선택지를 한 열로 배치하고 `확실한 사실 → 불확실한 사실 → 관계 문장 → 확인` 순서를 유지한다.
- 드래그 입력은 `대상 선택 → 슬롯 선택 → 확인`으로 대체한다.

### 22.3 신체 핫스폿 표시

```text
consent_scope 없음
→ servant body hotspot 미등록

consent_scope 유효
→ 44×44 논리 px 이상 hit_shape
→ owner 문양+부위명+행동 동사
→ 큰 타깃 설정 시 56×56 논리 px
```

- 동의 전에는 잠금 아이콘이나 흐릿한 포트로 숨은 보상을 암시하지 않는다.
- 포커스 확대가 신체를 선정적으로 크롭하지 않으며 목적 부위·도구·안전 절차를 함께 보여 준다.
- 겹친 귀·날개·소품 핫스폿은 대상 목록에서 부위명과 목적을 선택한다.
- 화면 읽기 순서는 `인물명 → 부위 → 목적 → 동의 상태 → 가능한 행동`이다.
- 접근성 설정은 `consent_scope`, 수리 결과, 관계 outcome을 생성하거나 연장하지 않는다.

### 22.4 관계 선택 중립 QA

| QA ID | 조건 | 기대 |
| --- | --- | --- |
| `QA-REL-WIN-01` | 키보드 전용 | E3_1~E3_5 진입·퍼즐·선택·이탈·재진입 완료 |
| `QA-REL-WIN-02` | 마우스 hover만 수행 | outcome·bond·alert 미확정 |
| `QA-REL-WIN-03` | UI·글자 200% | 선택지 정보량·확인 버튼·기록 링크 유지 |
| `QA-REL-WIN-04` | 선택 직전 `Alt+Tab` | 중립 포커스와 미확정 상태 유지 |
| `QA-REL-A11Y-01` | 색 제거+음량 0+모션 감소 | 5인 퍼즐·고백·선택 의미 유지 |
| `QA-REL-A11Y-02` | 큰 핫스폿+화면 읽기 | 동의한 부위만 순회하고 목적·행동 낭독 |
| `QA-REL-A11Y-03` | 모든 보조 설정 사용 | 기본 설정과 같은 outcome·기록·관계 결과 |
