...
P2의 창문·청소 도구는 한 사건 안에서만 물리 상태를 가지는 자식 상호작용이다. 따라서 62개 canonical 오브젝트 수를 임의로 늘리지 않고 `P2`가 상태 writer를 소유한다. 문서 15는 첫 조사·반복·오답·RESET 표현을 소유하고, 정답 순서와 완료 판정은 문서 06의 P2가 소유한다.
...
| `P3_OUTER_CLOCK_PREVIEW` | `B3_A`·B3 외부 시계 하우징 | 멈춘 초침이 오배치 순간 한 칸 거꾸로 움직인다. | 정식 ID는 `GGB-CNF-2026-0018` 해결 전 발급하지 않으며 `OBJ_LIBRARY_RECORD_CLOCK`으로 라우팅하지 않는다. |
...
아래 ID는 62개 canonical 오브젝트에 추가하지 않는다. 독립 세이브 객체가 아니라 각 사건의 로컬 상태를 읽고 쓰는 확대 화면 부품·공간 핫스폿이다.
...
| P3 경계 | `P3_INNER_GLASS_DOOR`, `P3_OUTER_CLOCK_PREVIEW` | 없음 | `LOCK_LIBRARY_INNER`, B3-A | 상위 사건 규칙 |
...
| `08` 메인 퍼즐 | B3~F0의 패널·카드·도구·월드 진입점 | 퍼즐 내부 부품은 문서 08 writer와 `ART_PUZ_*`가 소유하고, 문서 15는 월드 표면과 fallback만 소유한다. 외부 서고 시계 정식 ID는 `GGB-CNF-2026-0018`에서 계속 추적한다. |
| `09` 정보·일지 | 외부/내실 서가·일지·책상·색인·경계 시계·벽감·이중 바닥 | canonical 9종과 P3 자식 반응으로 충족한다. |
...
- `GGB-CNF-2026-0018` 외에 새로 열린 미해결 오브젝트 충돌은 없다.
...
| 문서 | 후속 반영 | 우선순위 | 상태 |
| --- | --- | --- | --- |
| `04_전체이벤트리스트_상태표.md` | 62개 canonical 대상, interaction role, 금지 쓰기·QA | P1 | 완료 |
...
| `09_이벤트상세_04_정보조사_일지복원.md` | 서재 9종 canonical ID, B2 주의·은신 큐 | P1 | 완료 |
