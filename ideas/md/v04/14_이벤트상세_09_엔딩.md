...
| 저장 스키마 | [17 상태·Godot 데이터](17_상태변수_이벤트ID_Godot데이터구조.md) | 진행 schema 12·profile v1·엔딩 재개 계약 |
...
| `16_색상연출_UI_접근성규칙.md` | 엔딩 필기·호흡·현실 수첩·S5 외형, Windows 창·입력·확대, EDC 선택 중립 접근성 | 완료 |
| `17_상태변수_이벤트ID_Godot데이터구조.md` | `ending_run`·`ending_meta`·새 ID·저장 지점, 이후 오브젝트 반응을 포함한 schema 12 | 완료 |
...
## 18. 엔딩 오브젝트 canonical ID 역참조

### 18.1 현실 엔딩 필수 오브젝트

| 순서 | object_id | 기능 | 완료 상태 |
| --- | --- | --- | --- |
| 자유 | `OBJ_REALITY_HAND` | 손가락·관절·실제 통증 확인 | EDR_BODY_CHECK 하위 논리 집합 |
| 자유 | `OBJ_REALITY_BREATH_MONITOR` | 자동 호흡과 자기 호흡의 분리 확인 | EDR_BODY_CHECK 하위 논리 집합 |
| 자유 | `OBJ_REALITY_RESTRAINT` | 비상 손잡이·혼자 풀 수 있는 구조 확인 | EDR_BODY_CHECK 하위 논리 집합 |
| 이후 | `OBJ_REALITY_FIELD_NOTEBOOK` | 공동 인계 수첩의 표지·첫 72시간 확인 | `ending_run.required_interactions_seen` |
| 마지막 | `OBJ_REALITY_EXIT_PANEL` | 필수 조사 확인 뒤 시설 밖으로 이동 | `ending_run.required_interactions_seen` |

신체 확인 3종 중 2종과 수첩의 표지·첫 72시간을 확인하면 출구 패널이 활성화된다. `SUBJECT_HANDOFF_PAGE`와 owner 탭은 선택형 후속 정보다. 생략된 신체 오브젝트는 부정적 결산을 만들지 않는다.

### 18.2 현실 엔딩 선택 오브젝트

| object_id | 짧은 후속 이야기 | 확정하지 않는 것 |
| --- | --- | --- |
| `OBJ_REALITY_CAPSULE_GLASS` | 안쪽 손자국과 바깥 정비 자국의 시간 차 | 현재 구조자 존재 |
| `OBJ_REALITY_EMPTY_STAND` | 빈 보존대의 규격과 제거 흔적 | 다른 생존자의 생사 |
| `OBJ_REALITY_TOOLBOX` | 비어 있는 칸과 당장 쓸 공구 두 개 | 장기 생존 성공 |
| `OBJ_REALITY_DRY_PLANTER` | 실제 토양값과 시뮬레이션 봄의 차이 | 지표 회복 불가능 |
| `OBJ_REALITY_NAME_WALL` | 다섯 연구원의 이름과 일부 판독 불가 이름 | 모든 연구원의 전말 |
| `OBJ_REALITY_DISTANT_SIGNAL` | 먼 점멸 주기 기록 | 사람·자동 장치·오류 중 정답 |

선택 조사는 `ending_run.optional_interactions_seen`에만 기록된다. 0개·6개 어느 쪽도 엔딩 종류, 관계 결산, 크레딧 순서를 바꾸지 않는다.

### 18.3 잔류 엔딩 필수 헌장

| object_id | 필수 확인 |
| --- | --- |
| `OBJ_STAY_MEMORY_CHARTER` | 진실 기억·감금 책임·변경 동의 원칙 |
| `OBJ_STAY_APPEARANCE_CONTROL` | `layered`·`contextual` 외형을 언제든 바꿀 수 있음 |
| `OBJ_STAY_AUTONOMY_CHARTER` | 사용인 역할을 명령이 아니라 제안으로 전환 |

세 헌장은 잔류를 안전한 원상복구로 만들지 않는다. `S5_STABILIZED_FRACTURE`는 파열과 진실 기억을 보존한 새 운영 상태다.

### 18.4 S5 선택 오브젝트

| 공간 | object_id | 후속 기능 |
| --- | --- | --- |
| 중앙홀 | `OBJ_STAY_CLOCK` | 열세 번째 칸의 수동 동기화 표시 |
| 중앙홀 | `OBJ_STAY_SCHEDULE` | 명령형 일정이 제안·빈칸으로 바뀐 흔적 |
| 중앙홀 | `OBJ_STAY_FRONT_DOOR` | 현관 투영 범위와 `MODEL BOUNDARY` |
| 중앙홀 | `OBJ_STAY_CARPET` | 고딕 문양과 시설 배선을 함께 표시 |
| 중앙홀 | `OBJ_STAY_CALL_CORD` | 호출이 명령이 아닌 연결 요청으로 바뀜 |
| 중앙홀 | `OBJ_STAY_NOTEBOOK` | 현재 합의와 다음 변경 가능성 기록 |
| 식당·인물 | `OBJ_STAY_STAIN` | 지워지지 않은 얼룩과 기억 보존 |
| 식당·인물 | `OBJ_STAY_TEA` | 완벽하지 않은 온도와 루카의 자발적 수정 |
| 식당·인물 | `OBJ_STAY_SEASON_WINDOW` | 선택 가능한 계절과 모델 경계 |
| 식당·인물 | `OBJ_STAY_PORTRAIT_LABEL` | 기능명이 아닌 다섯 이름 |
| 식당·인물 | `OBJ_STAY_RAPIER` | 에드가가 내려놓은 통제 권한의 상징 |

S5 선택 오브젝트는 `ending_run.optional_interactions_seen`만 쓴다. `OBJ_STAY_APPEARANCE_CONTROL`의 모드 변경은 가역 UI 설정이며 서사 선택을 새로 커밋하지 않는다.

### 18.5 선택 중립과 관계 overlay

- 현실의 먼 신호와 잔류의 따뜻한 차 중 어느 쪽도 최종 정답처럼 강조하지 않는다.
- 완료 관계는 필기 습관·짧은 음성·소품 반응을 더하지만 오브젝트의 사실을 바꾸지 않는다.
- 미완료 관계도 이름·좌석·기능 채널이 소실되지 않는다.
- F0-E 의향과 F3 조사 순서는 엔딩 오브젝트의 기본 포커스·활성 순서를 바꾸지 않는다.
- 엔딩 오브젝트는 `final_decision`을 읽을 수만 있고 다시 쓸 수 없다.

### 18.6 엔딩 오브젝트 QA

| QA ID | 검증 |
| --- | --- |
| `QA-END-OBJ-01` | 현실 신체 확인 임의 2-of-3과 수첩 뒤 출구 활성 |
| `QA-END-OBJ-02` | 현실 선택 조사 0개·6개의 최종 프레임 동일 |
| `QA-END-OBJ-03` | S5 헌장 3종은 필수, 중앙홀·식당 조사는 선택 |
| `QA-END-OBJ-04` | layered·contextual 전환으로 final_decision 불변 |
| `QA-END-OBJ-05` | LOW·MID·HIGH·ALL에서 canonical object 사실 동일 |

## 19. 문서 16 Windows 엔딩 UI 동기화

### 19.1 EDC 화면 구성

```text
상단: 현재 상태 `FINAL DECISION: UNSET`
중앙: SUBJECT 수첩대와 확실·불확실 정보
좌우: 기상 절차 / 안정화 루프 장치
하단: 다시 조사 / 대사 로그 / 설정 / 선택 확인
```

| 항목 | 기상 절차 | 안정화 루프 | 허용 오차 |
| --- | ---: | ---: | ---: |
| 버튼 면적 | 100% | 100% | ±2% |
| 중앙에서 거리 | 동일 | 동일 | ±4 논리 px |
| 기본 명도 단계 | 동일 | 동일 | 차이 없음 |
| 위험 아이콘 | 1 | 1 | 차이 없음 |
| 확실·불확실 정보 | 각 2~4줄 | 각 2~4줄 | ±1줄 |
| 확인 횟수 | 1 | 1 | 차이 없음 |

- EDC 진입 기본 포커스는 두 선택지가 아니라 중앙 SUBJECT 수첩대다.
- 현실에 차가운 청색, 잔류에 따뜻한 황색을 집중해 선택 가치를 유도하지 않는다.
- F0 임시 의향, 관계 등급, 마지막 장치 조사, 포인터 위치는 카메라·버튼·포커스 순서를 바꾸지 않는다.
- `Tab` 순회는 중앙 설명→두 장치의 대칭 정보→취소·로그·설정 순으로 구성한다.
- 선택 확정 전 `Esc`는 F3 완료 지점으로 돌아가며 `FINAL_SLEEP_LOCK`을 유지한다.
- 마우스 hover만으로 장치가 선택되거나 확인창이 열리지 않는다.

### 19.2 Windows 표시·포커스 상실

- 기준 1920×1080, 최소 1280×720, 2560×1440·3840×2160 확대를 검증한다.
- 16:9 밖의 여백에 엔딩 힌트·오브젝트·미리 보기 장면을 노출하지 않는다.
- UI·글자 200%에서는 두 선택지를 세로로 쌓지 않고, 동일한 좌우 카드 안에서 각 카드 내용을 스크롤 가능하게 만든다.
- 카드 스크롤 위치가 달라도 선택 확정 버튼의 면적·확인 횟수는 같다.
- `Alt+Tab`, 최소화, 창 포커스 상실 시 자동 진행과 확정 입력을 멈춘다.
- EDC 원자 커밋 중 종료 요청은 커밋 완료 또는 전체 롤백 뒤 처리한다.
- 해상도·창 모드 변경 확인창은 EDC 선택 확인창과 분리하고, 원래 중립 포커스로 돌아온다.

### 19.3 ALL 의식·현실 입력

| 장면 | 마우스 기본 | 키보드·운동 보조 | 결과 |
| --- | --- | --- | --- |
| 이름 인증 | 문양·이름 순차 클릭 | 목록에서 `Enter` | 동일 |
| 서명 필기 | 한 번 긋기 | 길게 누르기 또는 자동 추적 | 동일 |
| 현실 첫 호흡 | 관찰만 | 입력 없음 | 자동 성공 |
| 신체 확인 | 3종 중 2종 클릭 | `Tab` 순회 후 `Enter` | 동일 |
| 현실 수첩 | 페이지·탭 클릭 | 방향키·PageUp/Down | 동일 |
| 출구 패널 | 확인 클릭 | `Enter` | 동일 |

- 필기 정확도·속도·궤적은 엔딩 성공이나 관계 결과에 쓰지 않는다.
- 현실 첫 호흡에 클릭 연타, 리듬 입력, 마이크 입력을 요구하지 않는다.
- 숨·심박음 민감 설정에서는 음량을 낮추고 감각 자막·파형으로 대체한다.
- 현실 수첩은 작성자→제목→첫 72시간 요약→원문→뒤로 순으로 화면 읽기한다.
- `SUBJECT_HANDOFF_PAGE`는 `프린터 출력물`이라고 명시해 시뮬레이션 수첩과 혼동하지 않는다.

### 19.4 S5 외형·오브젝트

- `layered`와 `contextual`은 Windows 설정 메뉴와 `OBJ_STAY_APPEARANCE_CONTROL` 양쪽에서 바꿀 수 있다.
- 두 모드는 표시 설정이며 `final_decision`, 관계, 필수 헌장 완료 Set을 변경하지 않는다.
- 레이어 투명도와 윤곽 복잡도를 조절해도 시설 라벨과 `FILTER: DISPLAY ONLY`를 숨기지 않는다.
- 200% UI에서는 중앙홀·식당 선택 오브젝트 목록을 공간별 접이식 목록으로 제공한다.
- 선택 오브젝트를 전부 생략해도 마지막 화면과 크레딧에 도달한다.
- 관계 overlay는 owner 문양·이름·소품을 추가하지만 필수 핫스폿 순서를 바꾸지 않는다.

### 19.5 Windows·접근성 QA

| QA ID | 조건 | 기대 |
| --- | --- | --- |
| `QA-END-WIN-01` | 키보드 전용 | EDC→ALL 선택 장면→EDR·EDS 각각 완료 |
| `QA-END-WIN-02` | EDC 중 `Alt+Tab` | 자동 확정 없음, 중앙 중립 포커스 복귀 |
| `QA-END-WIN-03` | 1280×720·UI/글자 200% | 두 선택 정보와 확인·취소 버튼 접근 가능 |
| `QA-END-WIN-04` | EDC 커밋 도중 종료 | 전부 롤백 또는 확정 분기로 재개 |
| `QA-END-A11Y-01` | 색 제거+음량 0+모션 최소 | 두 선택·ALL 의식·두 에필로그 완료 |
| `QA-END-A11Y-02` | 자동 필기·호흡 입력 없음 | 수동 경로와 같은 엔딩 결과 |
| `QA-END-NEUTRAL-01` | F0 의향·관계·조사 순서 전 조합 | 버튼 면적·거리·정보량·확인 횟수 동일 |

## 20. 문서 17 엔딩 저장·프로필 동기화

### 20.1 세 저장 수명

| 데이터 | 파일 | 버전 | 롤백 범위 |
| --- | --- | --- | --- |
| 본편·엔딩 실행 | `user://saves/{slot_id}/progress.json` | schema 12 | 현재 슬롯 |
| F3 재선택 사본 | `user://saves/{slot_id}/f3_reselect.json` | schema 12 snapshot | 원본 슬롯과 분리 |
| 엔딩 열람 메타 | `user://profile/ending_meta.json` | profile 1 | 모든 슬롯 공통 |

접근성은 `user://profile/accessibility.json` profile 1이며 엔딩 결과와 별도다. 표시 설정 손상·초기화가 엔딩 seen이나 본편 분기를 지우지 않는다.
...
- 세 배열은 논리 집합일 때 중복 없는 `Array[StringName]`으로 보유하고 JSON 저장 전 사전순 정렬한다.
- `completed_nodes`에는 현재 분기의 실행 node만 넣는다.
- required·optional interaction은 일반 오브젝트 첫 조사 이력과 합치지 않는다.
- `current_node_id`는 다음 대사 줄이 아니라 안전한 사건 node를 가리킨다.
- `ending_appearance_mode`는 잔류 표시만 바꾸며 분기·관계·메타를 쓰지 않는다.

### 20.3 F3 재선택 사본

`SAVE_F3_COMPLETE`를 만들 때 선택 미확정 상태의 독립 사본을 생성할 수 있다.

```yaml
f3_reselect_snapshot:
  source_slot_id: slot_01
  source_save_point_id: SAVE_F3_COMPLETE
  final_decision: unset
...
- 크레딧 뒤 “다른 선택 확인”을 고르면 사본을 임시 회수 슬롯로 로드한다.
- 메인 progress를 자동 덮어쓰지 않는다.
- `ending_meta`의 기존 seen은 유지한다.
- 관계·기록·F0 임시 의향은 F3 완료 시점 snapshot을 그대로 사용한다.
- 사본에서 EDC 전 종료하면 다시 F3_COMPLETION으로 복귀한다.
- 사본에서 확인한 두 번째 엔딩도 정상적으로 profile seen을 추가한다.

### 20.4 EDC와 엔딩 node 저장

```text
SAVE_F3_COMPLETE
→ EDC_FINAL_DECISION atomic commit
→ SAVE_ENDING_BRANCH
→ ED_ALL_CEREMONY 선택 삽입
→ EDR_ENTRY 또는 EDS_ENTRY
→ SAVE_ENDING_NODE 반복
→ ENDING_META_COMMIT
...
| 단계 | transaction 보장 |
| --- | --- |
| EDC | final decision·choice relation·branch ID·committed를 함께 씀 |
| ALL 의식 | 확정 분기를 읽기만 하며 실패·분기 변경 없음 |
| 필수 node | node 완료와 required interaction을 함께 씀 |
| 선택 조사 | optional logical set만 갱신 |
| FINAL_FRAME | 모든 required node 완료 확인 |
| META_COMMIT | 분기 seen·any seen·gallery·chapter select를 profile에 함께 씀 |

EDC transaction 중 종료는 전부 rollback 또는 `SAVE_ENDING_BRANCH`까지 완료된 상태만 허용한다. ALL 의식이나 선택 조사가 중단되어도 EDC를 다시 묻지 않는다.
...
- `ENDING_META_COMMIT`만 이 파일을 쓴다.
- profile 쓰기 실패 시 마지막 프레임에서 재시도하고 크레딧 전에 성공 여부를 확인한다.
- progress rollback으로 seen을 지우지 않는다.
- future profile version은 덮어쓰지 않고 파일을 보존한다.
- profile이 없으면 완료된 현재 엔딩에서 해당 분기 seen을 안전하게 생성한다.

### 20.6 엔딩 데이터 QA

| QA ID | 조건 | 기대 |
| --- | --- | --- |
| `QA-END-DATA-01` | EDC write 단계마다 종료 | 결정·관계·branch·save point 전부 rollback 또는 완료 |
| `QA-END-DATA-02` | ALL 의식 중 종료 | 확정 분기 유지·첫 미완료 인증 재개 |
| `QA-END-DATA-03` | required node 순서 변경 | logical set 동일·FINAL_FRAME 조건 동일 |
| `QA-END-DATA-04` | 선택 조사 0개·전부 | final decision·settlement·meta 결과 동일 |
| `QA-END-DATA-05` | F3 사본에서 반대 엔딩 완료 | 메인 progress 자동 덮어쓰기 없음·seen 2종 보존 |
| `QA-END-DATA-06` | ending meta 손상·progress 정상 | 현재 엔딩으로 profile 복구·본편 상태 유지 |
| `QA-END-DATA-07` | 접근성 profile 삭제 | 엔딩 node·분기·seen 불변 |
| `QA-END-DATA-08` | future schema·profile 로드 | 파일 덮어쓰기 없이 지원 불가 안내 |
